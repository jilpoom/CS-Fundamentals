...
$$101.11_{(2)} = 1 \times 2^2 + 0 \times 2 ^1 + 1 \times 2^{-1} + 1 \times 2^{-2} = 4 + 0 + 1 + \frac {1}{2} + \frac {1}{4} = \frac {53}{4}$$
...
![Illustration of rounding modes for dollar rounding](./static/figure%202-36.PNG)
...
우선, $\infty$, $-0$, $NaN$과 같은 특수 값의 정의를 미리 정한다.
  - E.G.) $1/-0$ = $-\infty$
...
$x + ^fy$이라는 연산을 $Round(x + y)$라고 할 때, 다음의 교환 법칙을 만족한다.
